...
```mermaid
graph TD
    G[Backend] --> F(Spring&Django)
    F[Spring&Dajango] --> A(Spring)
    F[Spring&Dajango] --> B(SpringMVC)
    F[Spring&Dajango] --> C(SpringBoot)
    F[Spring&Dajango] --> D(SpringCloud)
    F[Spring&Dajango] --> E(SpringAI)
    F[Spring&Dajango] --> F(Django)
    G --> H(MySQL)
    G --> I(Redis)
    G --> J(MongDB)
    G --> K(ELK)
    K[ELK] --> L(ElasticSearch)
    K --> M(Kibana)
    K --> N(Logstash)
    H[MySQL] --> O(JDBC)
    H --> P(MyBatis)
